...
build_package() {
	...
	cp -rf ./LICENSE ./.sites ./.imgs ./Dcracker.sh ./package/"$path_part"usr/opt/$PACKAGE_NAME
	cp -rf ./.package/launch.sh ./package/"$path_part"usr/bin/$PACKAGE_NAME
	chmod 755 ./package/DEBIAN
	dpkg-deb --build ./package $PACKAGE_NAME\_$DCRACKER_VERSION\_$PACKAGE_ARCH.deb
}
